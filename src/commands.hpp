...
bool processBootBin(const std::string& bootname, unsigned int& bootsize,
                    std::vector<unsigned int>& hackoffsets, std::fstream& boot);

// Applies the LBA hack to boot at every offset found by processBootBin
// (no-op for Windows CE binaries, which don't need it).
bool hackBootBin(std::fstream& boot, const std::vector<unsigned int>& hackoffsets,
...
bool loadIpBin(const std::string& ipname, char* buffer);
...
bool createHackedIpBin(const std::string& ipname, char* iphackbuf,
                       unsigned int bootsize, std::fstream& boot);
...
int runBinhackBoot(const std::string& bootname, unsigned int lba);
...
// modified.
int runBinhackIp(const std::string& bootname, const std::string& ipname);

// Patch both BOOT.BIN and IP.BIN (classic BINHACK behavior).
int runBinhack(const std::string& bootname, const std::string& ipname, unsigned int lba);
...
int runHack0(const std::string& bootname, unsigned int lba, unsigned int oldLba);
int runHack(const std::string& bootname, unsigned int lba, unsigned int oldLba);
int runHack2(const std::string& bootname, unsigned int lba, unsigned int oldLba);
int runHack3(const std::string& bootname, unsigned int lba, unsigned int oldLba);
int runDahack(const std::string& bootname, unsigned int lba, unsigned int oldLba);
...
int runWinceCddaFix(const std::string& ipname);
